  ...
  <xsl:choose>
    <xsl:when test="@ballot-text">
          <fo:table-cell number-columns-spanned="3">
            <fo:block text-align="justify">Бюллетень для голосования по <xsl:value-of select="@ballot-text"/></fo:block></fo:table-cell>
    </xsl:when>
    <xsl:otherwise>
          <fo:table-cell>
            <fo:block text-align="center">Бюллетень для голосования по</fo:block>
          </fo:table-cell>
          <fo:table-cell><fo:block text-align="center"><xsl:value-of select="count(preceding-sibling::topic) + 1"/></fo:block></fo:table-cell>
          <fo:table-cell>
      ...
      <fo:table-body>
        <fo:table-row>
          <fo:table-cell border-bottom="thin solid black" vertical-align="bottom">
            <fo:block text-align="center"><xsl:value-of select="/meeting/date"/></fo:block>
          </fo:table-cell>
          <fo:table-cell><fo:block/></fo:table-cell>
          <fo:table-cell border-bottom="thin solid black">
            <fo:block text-align="center"><xsl:value-of select="/meeting/place"/></fo:block>
          </fo:table-cell>
        </fo:table-row>
        <fo:table-row>
          <fo:table-cell>
      ...
      <fo:table-body>
        ...
        <fo:table-row>
          ...
          <fo:table-cell><fo:block/></fo:table-cell>
            ...
            <fo:block text-align="center" border-bottom="thin solid black" margin-top="0.5cm"><xsl:value-of select="$topic"/></fo:block>
            <fo:block text-align="center" font-size="10pt">(формулировка решения, поставленного на голосование)</fo:block>
            <fo:block text-align="center" border-bottom="thin solid black" margin-top="0.5cm"><xsl:value-of select="text()"/></fo:block>
    ...
    <fo:table  width="17cm" table-layout="fixed" margin-top="0.5cm">
      ...
      <fo:table-body>
        <fo:table-row>
          ...
          <fo:table-cell border-bottom="thin solid black">
            <fo:block text-align="center"><xsl:value-of select="../claim"/></fo:block>
          </fo:table-cell>
        </fo:table-row>
      </fo:table-body>
    </fo:table>
